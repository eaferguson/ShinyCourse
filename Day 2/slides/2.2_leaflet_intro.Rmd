...
popupInfo <- paste("Species: ", map_data$species, "<br>",
                   "Date: ", map_data$date, "<br>",
                   "Age: ", map_data$age, "<br>",
                   sep = " ")
...
leaflet() %>% 
  ...
  addPolygons(data = region_shp, fillColor = myPal(log(region_shp$density)),
  ...
  addLegend(position = "topright", title = "Human<br>density<br>(log)",
            pal = myPal, values = log(region_shp$density), opacity = 0.9,
            labFormat = labelFormat(big.mark = "")) %>% 
