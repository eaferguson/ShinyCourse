...
Whenever we show example code, ui code will have a <span class="ui-background">yellow background</span> and server code with have a <span class="server-background">blue background</span>.
...
The plot is made interactive on the server side by using the user's selection as an input in our plot code.
<br><br>
<b>server</b>
...
- The previous examples used ``render()`` functions, which are **reactive**.  
...
- If we want to process the data based on user input (e.g. filter, summarise), we need wrap the code within a **reactive function**.  
...
<!----------------------------------------------------------------------------->
## Summary - section 1.4d server code {.smaller}

```{r, echo=TRUE, eval=FALSE, class.source="server-background"}
# ABOVE SERVER SECTION
...
<b style="color: red;">Issues adding app with pre-processing - will find fix! </b>
...
<!----------------------------------------------------------------------------->
## Summary - section 1.4e server code {.smaller}
