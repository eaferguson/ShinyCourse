...
- ``eventReactive()`` returns ``NULL`` util the button is pressed, which means any outputs that rely on it are hidden until the button is pressed...
...
- The difference is that it does not produce an output - it works by updating existing outputs

<!----------------------------------------------------------------------------->
## The ``reactiveValues()`` function

- The ``reactiveValues()`` function produce an object that acts like a list: you can store multiple values or objects to <em>slots</em>, that are accessed with the ``$`` symbol
