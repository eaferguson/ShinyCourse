...
- Any objects created in the function should be done with an ``=`` rather than the normal ``<-``.  
...
## The ``observeEvent()`` function
 ...
 - ``observeEvent`` watches for a specific user input, then runs the following code  
 - The input can be any of the widgets in your app, but this function is particularly helpful with an ``actionButton()``
