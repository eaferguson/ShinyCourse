...
In Shiny, we can let the users define what to plot. Using the drop down menu provided, they could choose to plot the sex or the specie.

user interface code here

which allows them to select the xaxis.

We can make the plot interactive on the server side by using the users selection as an input in our plot code

```{r, eval = F, echo = T}
ggplot() + 
  geom_bar(data=raw_data, aes_string(x=input$x_axis)) + 
  theme_classic()
```
